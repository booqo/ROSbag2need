...
class RosVideoWriter():
    ...
    def filter_image_msgs(self, topic, datatype, md5sum, msg_def, header):
        print(f"Checking topic: {topic} with datatype: {datatype}")
    ...
    def addBag(self, filename):
        ...
        for topic, msg, t in bag.read_messages(connection_filter=self.filter_image_msgs, start_time=self.opt_start, end_time=self.opt_end):
            try:
                if msg.encoding.find("jpeg")!=-1 :
                    if msg.encoding.find("8")!=-1 and (msg.encoding.find("rgb")!=-1 or msg.encoding.find("bgr")!=-1 or msg.encoding.find("bgra")!=-1 ):
                        if self.opt_display_images:
                            np_arr = np.fromstring(msg.data, np.uint8)
                            cv_image = cv2.imdecode(np_arr, cv2.CV_LOAD_IMAGE_COLOR)
                        self.write_output_video( msg, topic, t, MJPEG_VIDEO )
                    elif msg.encoding.find("mono8")!=-1 :
                        if self.opt_display_images:
                            np_arr = np.fromstring(msg.data, np.uint8)
                            cv_image = cv2.imdecode(np_arr, cv2.CV_LOAD_IMAGE_COLOR)
                        self.write_output_video( msg, topic, t, MJPEG_VIDEO )
                        ...
                        if self.opt_display_images:
                            ...
                            cv_image = cv2.imdecode(np_arr, cv2.CV_LOAD_IMAGE_COLOR)
                        self.write_output_video( msg, topic, t, MJPEG_VIDEO )
                    elif msg.encoding.find("32FC1") != -1:
                        # 处理32FC1的深度图像，将其转换为8位灰度图像
                        ...
                        print('unsupported jpeg format:', msg.encoding, '.', topic)
